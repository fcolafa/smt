 ...
 $(document).ready(function(e) {
        ...
        if($model->_guides){
            foreach ($model->_guides as $item){
                ...
                if($weight)
                    foreach($weight as $w)
                        if($item==$w->id_guide){
                            ...
                            echo "$('#we'+".$cont."+".$conw.").append('<h6>-'+'".$w->weightprovider." '+'".$w->weighttype."'+' '+".$w->amount_left."+' ('+'".$w->idWeightUnit->weight_unit_name."'+')'+'</h6>');"; 
                ...
                echo " $('#gui'+".$cont.").append('<h5>Guia:'+ '".CHtml::link(CHtml::encode($guide->num_guide." (".$guide->idUser->idCompany->company_name.")"), array('guide/view','id'=>$guide->id_guide,), array('target'=>'_blank'))."'+'</h5>');";
                     $cont++;
         ...
         <?php 
             ...
             var ist=0;   
                $("#Reception__guides option").each(function(){
                    ...
                    $("#gui"+indexguide).append("<h5>Guia:"+ ui.item["link"] +"</h5>");
...
            </table>
         ...
         -->
          <div class="row" >
              <p class="note"> al momento de cargar una guia, el sistema asumirà que se recibe la carga completa de dicha guia. No obstante si desea parcializar algunas de las cargas asociadas, debera apretar el boton "parcializar carga" e indicar cuales de ellas desea modificar. Para luego confirmar dicha edicion con el checkbox contiguo (nota: si alguno de los checkbox no esta seleccionado se asumira que dicha carga se recibio completa)  </p>
    <div class="">        
    <table  class="">
...
            </table>
	...
	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? Yii::t('actions','Notify') : Yii::t('actions','Notify'),array('name'=>'Notify','class'=>Yii::app()->params['btnclass'])); ?>
                <?php echo CHtml::submitButton($model->isNewRecord ? Yii::t('actions','Reject'): Yii::t('actions','Reject'), array('name' => 'Reject','class'=>Yii::app()->params['btnclass'],'confirm'=>'Esta seguro que desea rechazar las guias seleccionadas')); ?> 
	</div>
